...
function renderGlyph(group, glyph, x, y, scale) {
  const wrap = document.createElementNS("http://www.w3.org/2000/svg", "g");
  wrap.setAttribute("transform", `translate(${x} ${y}) scale(${scale})`);
  for (const d of glyph.p) wrap.append(pathEl(d, "stroke outer"));
  for (const d of glyph.p) wrap.append(pathEl(d, "stroke channel"));
  for (const d of glyph.p) wrap.append(pathEl(d, "stroke accent"));
  group.append(wrap);
}

function measureLine(line, scale, tracking) {
  let width = 0;
  for (const char of line) {
    const glyph = glyphs[char] || glyphs["-"];
    width += glyph.w * scale + tracking;
  }
  return Math.max(0, width - tracking);
}

function renderSystemmediaText(svg, text, options = {}) {
  const scale = options.scale || 1;
  const lineHeight = 150 * scale;
  const tracking = 13 * scale;
  ...
  style.textContent = `
    .outer { stroke: #050505; stroke-width: 18; stroke-linecap: round; stroke-linejoin: round; }
    .channel { stroke: #ffffff; stroke-width: 9.5; stroke-linecap: round; stroke-linejoin: round; }
    .accent { stroke: #19aeea; stroke-width: 3.8; stroke-linecap: round; stroke-linejoin: round; }
  `;
  svg.append(style);
  ...
  const lineWidths = lines.map((line) => measureLine(line, scale, tracking));
  const contentWidth = Math.max(260 * scale, ...lineWidths);
  const width = contentWidth + padding * 2;
  const height = Math.max(170, lines.length * lineHeight + padding * 1.25);
  let y = padding * 0.75;

  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index];
    let x = padding;
    if (align === "center") x += (contentWidth - lineWidths[index]) / 2;
    if (align === "right") x += contentWidth - lineWidths[index];
    for (const char of line) {
      const glyph = glyphs[char] || glyphs["-"];
      renderGlyph(group, glyph, x, y, scale);
      x += glyph.w * scale + tracking;
    }
    y += lineHeight;
  }

  svg.setAttribute("viewBox", `0 0 ${width} ${height}`);
  svg.dataset.align = align;
}

window.renderSystemmediaText = renderSystemmediaText;
